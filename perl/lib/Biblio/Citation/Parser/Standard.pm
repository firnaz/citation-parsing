package Biblio::Citation::Parser::Standard;
...
sub extract_metadata
{
	...
	my(@newtemplates) = ();
	foreach my $template (@$Biblio::Citation::Parser::Templates::templates)
	{
		$_ = $template;
		s/\\/\\\\/g;
		s/\(/\\\(/g;
	...
	{
		my $original = $Biblio::Citation::Parser::Templates::templates->[$index];
		if ($ref =~ /^$currtemplate$/)
		{
